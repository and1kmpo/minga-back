...
export default async (req, res, next) => {
  try {
    ...
    let all = await Manga.find(consultas, "title -_id description").sort(
      ordenamiento
    );
    if (req.query.page) {
    ...
    if (one.length > 0) {
      ...
      let prev_page =
        Number(pagination.page) === 1 ? null : Number(pagination.page) - 1;
      let next_page =
        Number(pagination.page) === pages ? null : Number(pagination.page) + 1;
